          ...
          tasks: [
            {
              ...
              script: [
                ...
                {
                  do: "Break the transport on purpose: add a stray print to server.py, call that tool, then remove the print.",
                  where: "server.py + terminal",
                  commands: "# 1. save as call_one.py:\nimport asyncio\nimport sys\n\nfrom mcp import ClientSession, StdioServerParameters\nfrom mcp.client.stdio import stdio_client\n\n\nasync def main():\n    server = StdioServerParameters(command=sys.executable, args=[\"server.py\"])\n    async with stdio_client(server) as (read, write), ClientSession(read, write) as s:\n        await s.initialize()\n        result = await asyncio.wait_for(s.call_tool(\"list_facilities\", {\"ward\": 1}), timeout=10)\n        print(result.content[0].text[:200])\n\n\nasyncio.run(main())\n\n# 2. temporarily add this as the first line inside list_facilities() in server.py:\n#        print(\"listing facilities\")\n# 3. run:\n#        python call_one.py\n# 4. delete the print and run python call_one.py again",
                  expect: "With the print in place: an error instead of a facility list — the client cannot parse \"listing facilities\" as a protocol message, and the call fails or times out after 10 seconds. Without it: the first facilities in ward 1 as JSON.",
                  point: "With the stdio transport, stdout is the protocol wire. One stray print corrupts the stream. That is why server.py has a log() helper that writes to stderr, and it is the most common reason a first MCP server \"just doesn't connect\".",
                },
                {
